...
@app.post("/detect-pair", response_model=PairDetectionResponse)
async def detect_pair(request: PairDetectionRequest):
    """
    Detect which text in a pair is real vs AI-generated.
    
    Returns which text (1 or 2) is more likely to be real.
    """
    try:
        # Prepare model slug
        model_slug = request.model_name.replace("/", "_")
        ...
        logger.info("Extracting features for text pair...")
        
        all_layer_embeds_1 = extractor.get_all_layer_embeddings(
            [request.text1.strip() or " "],
            pooling=request.pooling,
            batch_size=1
        )
        all_layer_embeds_2 = extractor.get_all_layer_embeddings(
            [request.text2.strip() or " "],
            pooling=request.pooling,
            batch_size=1
        )
        
        features_1 = all_layer_embeds_1[request.layer]
        features_2 = all_layer_embeds_2[request.layer]
        
        # Make predictions for both texts
        results_1 = detector.predict(features_1, return_probabilities=True, return_distances=False)
        results_2 = detector.predict(features_2, return_probabilities=True, return_distances=False)
        
        # Parse results
        pred_1 = int(results_1[0][0])
        prob_1 = float(results_1[1][0])
        pred_2 = int(results_2[0][0])
        prob_2 = float(results_2[1][0])
        
        # Determine which text is more likely real
        prob_real_1 = 1 - prob_1
        prob_real_2 = 1 - prob_2
        
        real_text_id = 1 if prob_real_1 > prob_real_2 else 2
        confidence = abs(prob_real_1 - prob_real_2)
        
        return PairDetectionResponse(
            real_text_id=real_text_id,
            text1_prediction=pred_1,
            text2_prediction=pred_2,
            text1_probability=prob_1,
            text2_probability=prob_2,
            confidence=confidence
        )
        
    except Exception as e:
        logger.error(f"Pair detection failed: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={
                "error": "Pair detection failed",
                "message": str(e)
            }
        )
